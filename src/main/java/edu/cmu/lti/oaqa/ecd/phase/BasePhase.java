...
public final class BasePhase extends JCasMultiplier_ImplBase {
	...
	public void initialize(UimaContext ctx)
		...
		String pp = (String) ctx
				.getConfigParameterValue("persistence-provider");
		if (pp == null) {
		...
		this.persistence = BaseExperimentBuilder.loadProvider(pp,
				PhasePersistenceProvider.class);
		this.phaseName = (String) ctx.getConfigParameterValue("name");
		this.phaseNo = (Integer) ctx
				.getConfigParameterValue(QA_INTERNAL_PHASEID);
		...
		Boolean lazyLoadOpts = (Boolean) ctx
				.getConfigParameterValue(LAZY_LOAD_KEY);
		...
		if (size() == 0) {
			throw new ResourceInitializationException(
					new IllegalArgumentException("Phase: " + toString()
							+ " provided no options"));
		}
		...
		int stageId = (Integer) ctx
				.getConfigParameterValue(BaseExperimentBuilder.STAGE_ID_PROPERTY);
		persistence
	...
	// We will call the process() method for each of the annotator on this
	// phase,
	...
	public JCas next() throws AnalysisEngineProcessException {
		...
		try {
			...
			try {
				...
				Trace trace = ProcessingStepUtils.getPartialTrace(
						prevTrace.getTrace(), getPhaseNo(), optionId);
				if (!loadCasFromStorage(nextCas, trace, sequenceId)) {
					process(ae, nextCas, prevCasId, prevTrace, optionId,
							sequenceId, trace);
				}
				// TODO:Should this be in a final clause?
				// Otherwise how would we move on to the next annotator in case
				// of exception?
				nextAnnotator++;
				return nextCas;
			} catch (Exception e) {
	...
	private AnalysisEngine getAnalysisEngine()
		...
		if (options != null) {
			...
			AnalysisEngine ae = AnalysisEngineFactory
					.createAggregate(optionDescriptions[nextAnnotator]);
			return ae;
		}
	}
	...
	private void process(final AnalysisEngine ae, JCas nextCas,
			String prevCasId, Trace prevTrace, String optionId,
			String sequenceId, Trace trace) throws IOException, SAXException,
			Exception {
		long a = System.currentTimeMillis();
		final String uuid = ProcessingStepUtils.getCurrentExperimentId(nextCas);
		final String key = ProcessingStepUtils.getExecutionIdHash(uuid, trace,
				sequenceId);
		try {
			...
			// Wrap the JCas to ignore downstream method invocation in case
			// component timeout.
			final DeferredTerminationJCasWrapper wrapped = new DeferredTerminationJCasWrapper(
					nextCas);
			// We are using a single thread executor for each phase, this means
			// that
			...
			Future<?> future = executor.submit(new Runnable() {
				...
				public void run() {
					try {
						// The execution id hash is added just before the future
						// is executed
						...
						// An async mechanism should be used to get the
						// exception
			...
			try {
				...
				// If the AE is taking too long and it times out it could be
				// either:
				// 1) It's reading from a resource or
				// 2) it's stuck on a iterative algorithm.

				// First send a termination event so the AE is aware that it's
				// been terminated.
				// Classes that inherit from TerminableComponent have access to
				// this information
				// and should check if the component has been terminated.
				PhaseEventBus.sendTerminateEvent(new TerminateEvent(key));
				// The cancel(true) method will call notify, any waiting IO
				// operations will be interrupted,
				// with an InterruptedIOException and the component be rendered
				// invalid.
				future.cancel(true);
				// We invalidate the JCas wrapper, so any subsequent calls to it
				// will throw an Exception
				wrapped.invalidate();
				long b = System.currentTimeMillis();
				storeException(b, e, key, ExecutionStatus.TIMEOUT);
				System.out.printf(
						"[%s]  Execution timed out for option: %s after %ss\n",
						sequenceId, optionId, (b - a) / 1000);
				// Finally re-throw the exception to allow the flow controller
				// do its job
				throw e;
			}
		} catch (Exception e) {
			...
			try {
				...
				System.out.printf(
						"[%s]  Execution failed for option: %s after %ss\n",
						sequenceId, optionId, (b - a) / 1000);
			} finally {
				nextCas.release();
			}
			throw e; // Re-throw exception to allow the Flow controller do its
						// job
		}
	}

	private void greedyCopy(JCas existingCas, JCas emptyCas) {
		// nothing 'greedy' about the default implementation, which copies
		// everything.
	...
	private void insertExecutionTrace(JCas jcas, final String optionId,
			final long startTime, final String prevCas, final Trace trace,
		...
		persistence.insertExecutionTrace(optionId, sequenceId, dataset,
				getPhaseNo(), uuid, startTime, getHostName(), trace.getTrace(),
				key);
	}

	private void storeCas(JCas jcas, final long endTime, final String key)
	...
	private void storeException(final long endTime, Exception e,
			final String key, ExecutionStatus status) throws IOException,
			SAXException {
	...
	private boolean loadCasFromStorage(JCas jcas, Trace trace, String sequenceId)
			throws SQLException {
		...
		final String hash = ProcessingStepUtils.getExecutionIdHash(
				experimentId, trace, sequenceId);
