...
public abstract class AbstractPhase extends JCasMultiplier_ImplBase {
  ...
  protected abstract void insertExecutionTrace(String optionId, int sequenceId, String dataset,
          Integer phaseNo2, String uuid, long startTime, String hostName, String trace, String key) throws IOException ;
  ...
  private void storeCas(JCas jcas, final long endTime, final String key) throws IOException,
    ...
    storeCas(bytes, ExecutionStatus.SUCCESS, endTime, key);
  }

  protected abstract void storeCas(byte[] bytes, edu.cmu.lti.oaqa.ecd.phase.ExecutionStatus success,
          long endTime, String key)  throws IOException ;
  ...
  private void storeException(JCas jcas, final long endTime, Exception e, final String key)
    ...
    storeException(bytes, ExecutionStatus.FAILURE, endTime, key);
  }

  protected abstract void storeException(byte[] bytes, ExecutionStatus failure, long endTime,
  ...
  private boolean loadCasFromStorage(JCas jcas, Trace trace, int sequenceId) throws SQLException {
    ...
    CasDeserializer deserializer = deserialize(jcas, hash);
  ...
  protected abstract CasDeserializer deserialize(JCas jcas, String hash) throws SQLException;
  ...
  protected abstract void insertExperimentMeta(String experimentId, int phaseNo, int stageId,
          int size);
