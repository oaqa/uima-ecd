...
public final class BasePhaseLoader {
	...
	List<AnalysisEngineDescription> loadOptions(String options) {
		...
		List<Map<String, Object>> ao = (List<Map<String, Object>>) yaml
				.load(options);
	...
	@SuppressWarnings("unchecked")
	void loadOption(Map<String, Object> description,
		...
		try {
			...
			if (type == HandleType.PIPELINE) {
				List<Map<String, Object>> pipeline = (List<Map<String, Object>>) first
						.getValue();
				List<AnalysisEngineDescription> options = createInnerPipeline(pipeline);
				aes.addAll(options);
			} else {
				String resource = (String) first.getValue();
				List<AnalysisEngineDescription> options = doLoadOptions(ResourceHandle
						.newHandle(type, resource));
				aes.addAll(options);
			}
		} catch (Exception e) {
	...
	@SuppressWarnings("unchecked")
	private List<AnalysisEngineDescription> createInnerPipeline(
		...
		for (Map<String, Object> map : pipeline) {
			try {
				...
				if (o instanceof String) {
					...
					ResourceHandle handle = ResourceHandle.newHandle(type,
							component);
					Set<AnalysisEngineDescription> local = Sets
							.newLinkedHashSet(doLoadOptions(handle));
					sets.add(local);
		...
		Set<List<AnalysisEngineDescription>> product = Sets
		...
		for (List<AnalysisEngineDescription> local : product) {
			...
			AnalysisEngineDescription aee = builder
					.createAggregateDescription();
			aee.getAnalysisEngineMetaData().setName(
					String.format("pipeline:(%s)", aeName));
			options.add(aee);
		}
		return options;
	}

	private List<AnalysisEngineDescription> doLoadOptions(ResourceHandle handle)
			throws Exception {
		List<AnalysisEngineDescription> aes = Lists.newArrayList();
		Map<String, Object> tuples = Maps.newLinkedHashMap();
		Class<? extends AnalysisComponent> comp = BaseExperimentBuilder
				.loadFromClassOrInherit(handle, AnalysisComponent.class, tuples);
		AnyObject crossParams = (AnyObject) tuples.remove(CROSS_PARAMS_KEY);
		if (crossParams == null) {
			AnalysisEngineDescription aeDesc = BaseExperimentBuilder
					.createAnalysisEngineDescription(tuples, comp);
			aes.add(aeDesc);
		} else {
			...
			for (List<Object> configuration : product) {
				...
				AnalysisEngineDescription aeDesc = BaseExperimentBuilder
						.createAnalysisEngineDescription(inner, comp);
				aes.add(aeDesc);
			}
		}
		return aes;
	...
	private void setInnerParams(List<String> paramNames,
			List<Object> configuration, Map<String, Object> inner) {
